      ...
      middleware = function expressRedisCache_Middleware (req, res, next) {
        ...
        self.get(name, domain.bind(function (error, cache) {
          ...
          if ( cache.length &&  cache[0].body != null ) {
            let maxAge = Math.round(((parseInt(cache[0].touched) - (+new Date())) + cache[0].expire*1000) /1000);
            res.setHeader('Cache-Control', 'private,max-age=' + maxAge);
            res.contentType(cache[0].type || "text/html");
            if(binary){ //Convert back to binary Buffer.from
              res.send(new Buffer.from(cache[0].body, 'base64'));
            }else{
          ...
          /** otherwise, cache request **/
            ...
            res.setHeader('Cache-Control', 'private,max-age=' + expirationPolicy(req, res));
